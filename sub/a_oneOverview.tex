...
\begin{itemize}
  ...
  \item If your version is the below the 3.0.1.0, read the \href{\#basic\_configuration\_installconfigure}{basic instructions to install and
    configure} \RR{} and Tinn-R:~ \texttt{it's a small and easy to follow document};
  \item Choose either~ \texttt{Rterm.exe or Rgui.exe: it takes one mouse click};
  \item Open \textit{Help/Example of script (R)}: \texttt{another mouse click};
  \item Use the \textit{R toolbar} to send e to control \RR{}: \texttt{it only takes one
...
\href{https://sourceforge.net/projects/tinn/files/TinnMDI/}{Tinn} is a small ASCII file
...
to control \href{http://cran.r-project.org}{R} running as Rterm.exe or
Rgui.exe (in SDI mode) and a whole lot of additional resources.

Tinn-R can also be thought of as feature-rich replacement of the basic
script editor provided with Rgui.exe. It provides syntax-highlighting, code
...
\begin{itemize}
  \item The ability to communicate with the \RR{} environment by sending
    ...
    \begin{itemize}
      \item Rterm.exe
      \item Rgui.exe. From Tinn-R version 8.02.01.01 onwards, Rgui.exe must be launched from within
       Tinn-R so that it is properly identified at startup
    \end{itemize}
  \item Projects:
    \begin{itemize}
  ...
  \item \LaTeX ~support:
    \begin{itemize}
      \item Edition
      \item Compilation
      \item Inverse DVI and PDF search.
    \end{itemize}
\end{itemize}
...
The Tinn-R Team have been developing Tinn-R project under the GPL since june of 2003.
All this time it was distributed as free software. Currently, there are corporative
alternatives (paid or free as RStudio) to lead with R environment. So, to cover the costs,
...
\begin{description}
  \item[Motivation to start Tinn-R:]
    ...
    \href{https://sourceforge.net/projects/tinn/files/TinnMDI/}{Tinn} as our initial
    platform.
...
includes indicating the Tinn-R version, your operating system (Windows 10,
Windows 11, etc) and language (English, French, Portuguese). If the bug is
related to an interface with \RR{}, please also indicate which version of \RR{}
you are using, as well as whether you are running Rterm.exe or Rgui.exe. Ideally,

...
\subsection*{6.1.1.1 (nov/25/2019)}
\begin{itemize}
  \item Some parts of the Object Pascal \textbf{pipe} were rewritten. It is now more fast, estable and robust.
  ...
  \item We are looking for the maximum freedom and efficiency to interact with the R interpreter.
    ...
    \begin{itemize}
      \item From now on, to send any prior line (or just any selection) to the R interpreter, you must press the
        \textbf{CTRL + ENTER} shortcut keys (not user configurable).
      \item In the latest line the user can use both: a simple \textbf{ENTER} or \textbf{CTRL + ENTER} to send the current line.
      \item The \texttt{R history} can be viewed (and filtered progressively as you type) in a new \texttt{SynEdit completion window}.
        The shortcut (not user configurable) is \textbf{CTRL + ALT + SPACE}.
    \end{itemize}
  ...
  \item From now on, the shortcut (not user configurable) \textbf{CTRL + ENTER} is associated to send line (or selection) from the Editor to the \RR{} interpreter.
    The previous use (send the current line and insert a new one) was removed. This was an old request from users but we believe it is no longer used much.
    Therefore, unless otherwise stated, we no longer wish to make this feature available.
\end{itemize}

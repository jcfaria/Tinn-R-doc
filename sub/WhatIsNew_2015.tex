...
\begin{itemize}
  \item Due to two portable versions (simple and compatible with Apps) some adjustments were made in the application source code and the project structure.
\end{itemize}
...
\begin{itemize}
  \item Bug(s) fixed:
    \begin{itemize}
      \item A bug related to \texttt{Rterm interface (IO and LOG)} and to the packages \texttt{car} and \texttt{rms} was fixed.
        The bug occurs because the pattern of messages on the pipe changes when both packages are loaded.
        Thanks to \texttt{Frank} for pointing it out.
      \item A bug related to \texttt{Update R mirrors} and to the countries \texttt{Germany, Spain and Sweden} was fixed.
    \end{itemize}
  \item The versions 4.0.3.1, 4.0.3.2 and 4.0.3.3 were restricted to testers: thanks for the tests and suggestions.
  \item From this version on, Tinn-R will be released in three flavors: \texttt{Tinn-R}, \texttt{Tinn-R Portable} and
    compatible with \texttt{PortableApps platform}.
  \item Some files of the folder \texttt{utils} were updated to meet the \texttt{Tinn-R Portable} project needs.
...
\begin{itemize}
  \item Bug(s) fixed:
    \begin{itemize}
      \item A bug related to the recent release (\RR{} 3.2.0) and to the installation of the necessary \texttt{TinnRcom} package was fixed.
      ...
      \item The file \texttt{Rinstall.R} located at the folder \texttt{utils}, where Tinn-R is installed, was updated to suppress
        the download of two packages (\texttt{Hmisc} and \texttt{R2Html}), which are no longer necessary to the \texttt{TinnRcom} package.
    \end{itemize}
...
\begin{itemize}
  \item Features Added:
    \begin{itemize}
      ...
      \item An experimental Hex viewer (\href{http://atviewer.sourceforge.net/atbinhex.htm}{ATBinHex}) was added to \texttt{Tools/Results/Hex viewer}. A bug is related to the UTF-8 BOM encoding file: the selection of (Char $\rightleftharpoons$ Hex), only in the first line, is not correctly associating the character with its corresponding Hex. It has its own pop-up menu with many (and useful) options.
    ...
    enabling the user to choose if the setwidth will be sent automatically when the panel width
    or font size is changed.
      \item A new option was added: \texttt{Project/Open demo}. The objective is to show the novice user what is a project and it's utility.
  ...
  \item Bug(s) fixed:
    \begin{itemize}
      \item A bug related to Sumatra (PDF viewer) and User guide (PDF), location of topics, was fixed. If you have it installed,
        please upgrade to 3.1.1 or higher (\href{http://www.sumatrapdfreader.org/prerelease.html}{SumatraPDF}).
      \item \texttt{Rterm} is now entirely updated after some changes in \texttt{Options/Application} or
      ...
      \item Some bugs related to Hide/Show resources of \texttt{Tools panel} were fixed.
    \end{itemize}
  \item The pre-release versions 4.0.0.0 to 4.0.1.5 were restricted to testers: thanks for the tests and sugestions.
  ...
  \item Prior backup (full or database) will no longer be compatible from this version on.
  \item A set of new icons was created specially to the project: thanks to \texttt{Philipe Silva Farias} for the hard work.
  \item The visual identity of the project changed: thanks to \texttt{Carolina Sartório Faria} for the work.
  ...
  \item The default shortcut to alternate the focus between Editor, IO and LOG was changed.
  ...
  \item The available font family was restricted to \texttt{True Type} and \texttt{FixedPitchOnly}.
  ...
  \item Tinn-R Editor - GUI for R Language and Environment is a project under GPL and distributed as freeware.
    Since creating and maintaining the project involve many costs, donations are welcomed!
\end{itemize}